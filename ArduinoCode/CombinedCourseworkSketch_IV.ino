...
void loop() {
  ...
  // Read CO2 sensor ALWAYS (regardless of mode) - for continuous monitoring
  if (millis() - lastSensorRead >= sensorInterval) {
    ...
    if (ens160.available()) {
      ...
      // ALWAYS print sensor readings to Serial Monitor
      Serial.println("========================================");
      Serial.print("AQI: "); Serial.print(aqi); Serial.print("\t");
      Serial.print("TVOC: "); Serial.print(tvoc); Serial.print("ppb\t");
      Serial.print("eCO2: "); Serial.print(co2_ppm); Serial.print("ppm");
      
      // Show current mode
      if (useLocalSensor) {
        Serial.println(" [CO2 Mode - controlling LEDs]");
      } else {
        Serial.println(" [MQTT Mode - monitoring only]");
      }
      
      // Only update LEDs if in local sensor mode
      if (useLocalSensor) {
        ...
        Serial.println("  → Not updating LEDs (in MQTT mode)");
      }
      Serial.println("========================================");
    }
  }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (length == payload_size) {
    ...
    useLocalSensor = false; // Disable CO2 sensor LED control (but keep monitoring)
