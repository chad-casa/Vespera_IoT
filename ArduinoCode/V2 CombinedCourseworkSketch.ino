...
void setup() {
  Serial.begin(115200);
  while(!Serial);
  Serial.println("Vespera with CO2 Sensor");
  ...
  Serial.println(useLocalSensor ? "Mode: CO2 sensor controlling colors" : "Mode: MQTT only");
}
...
void updateLEDsFromCO2(int co2_ppm) {
  ...
  // Fill the byte array with the CO2-based color for all LEDs
  for (int pixel = 0; pixel < num_leds; pixel++) {
    RGBpayload[pixel * 3 + 0] = r; // Red
    RGBpayload[pixel * 3 + 1] = g; // Green
    RGBpayload[pixel * 3 + 2] = b; // Blue
  }
  
  // Publish to MQTT
  if (mqttClient.connected()) {
    mqttClient.publish(mqtt_topic.c_str(), RGBpayload, payload_size);
    Serial.print("Published CO2-based color: RGB(");
